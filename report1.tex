...
When a program undergoes changes during development, updating the data flow information by doing exhaustive analysis is cost 
inefficient. In such cases, incremental data flow analysis, which modifies only those data flow information of a program that has
been changed, can be used. The savings made from avoiding the recomputation of the entire data flow information of a program can be
significant. The incremental analysis methods are known for bit-vector framework.
However, these methods are not directly applicable to non bit-vector framework. 
This report presents some challenges involved in doing incremental analysis for non bit-vector framework, and proposes several solutions towards the same.

This report also describes PRISM, an analyzer generator developed at Tata Research Development and Design Centre (TRDDC). 
We present \textit{Incremental Solver}, an enhanced version of the PRISM solver with the added capabilities 
of context sensitive incremental analysis. We discuss the changes made and the challenges faced in detail.
...
Another approach~\cite{inc2} is to first compare the old and new code version and generates a structural diff. This diff will give information about which nodes and edges in the 
