...
\begin{itemize}
 \item \textit{Re-initialization} which constructs the affected region.
 \item  \textit{Re-computation} which identifies boundary nodes and computes the information only for the affected region (set of affected node).
\end{itemize}
...
Late discovery of this issue has prevented any corrective measures. 
However, under the above stated assumptions, the only limitation that stems from reading from a
text file is that the result is stored in a non standard format by the solver.


\section{Performance Measurement}
We had to artificially add the changes to check the performance of the Incremental Solver.
...
\section{The Theory of Constant Propagation Analysis}
