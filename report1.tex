...
Following modules are used in bi-directional Solver.
\begin{itemize}
 \item DFStore: It stores context sensitive data flow information. It also provides an API to access the data. More details are given in Appendix.
 \item ValueContexts: It stores information about contexts formed during analysis.
 \item ContextTransition: It stores context transition graph which is a method of representing call strings.
 \item DFSolverLogger: This module provides utilities for logging, measuring performance parameters of the core solver.
 \item InterproceduralWorklist: This module manages work list and applies a priority scheme to the work list.
\end{itemize}
...
In this section, we describe the implementation of Incremental Solver. First we explained the architecture of the solver which is followed by implementation details. 
\subsection{Architecture of Incremental Solver}
Figure~\ref{fig:Architectur_incrementalPrism} shows an architecture of Incremental Solver.The \textit{Changed Node Detector} takes \textit{Old IR} and \textit{New IR} as inputs.
It compares two IRs and detects the changed nodes. Output of  the \textit{Changed Node Detector} is then passed to  the \textit{Preprocessor}. The \textit{Preprocessor} computes 
the affected region, a set of affected nodes, and a set of boundary nodes. The \textit{Solver} computes the data flow information for the affected region and passes the results
to the \textit{Merger}. \textit{Merger} will merge the solution and produce updated results. 
...
\putnode{n2}{n1}{40}{-5}{%
     \psframebox[linewidth=.3,framesep=1.7]{Changed Node Detector}}
...
\putnode{n8}{n5}{-56}{15}{%
     \psframebox[linewidth=.3,framesep=1.7]{Generated Files by Kulang Compiler}}
\putnode{n11}{n8}{33}{0}
...
\begin{itemize}
 \item IncrementalDriver: This driver takes two IR as an input and stores old IR information into OldIRWorklist module. 
 \item CreateAffectedRegion: 
 \item OldIRNode:
 \item OldIRWorklist:
\end{itemize}
